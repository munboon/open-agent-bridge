...
export function WorkspaceOverview({ projects, stale }: { projects: DirectoryProject[]; stale: boolean }) {
  ...
  return <section className="workspace-summary" aria-labelledby="workspace-summary-title">
    ...
    <dl className="workspace-summary-stats">
      <div><dt>Projects</dt><dd>{projects.length}</dd><p>{projects.filter(project => project.state === 'active').length} active · {projects.filter(project => project.state === 'paused').length} paused · {projects.filter(project => project.state === 'archived').length} archived</p></div>
      <div><dt>Agents connected</dt><dd>{stale ? 'Unavailable' : <>{connected.length}<small> / {agents.length}</small></>}</dd><p>{stale ? 'Waiting for the next successful update' : `${agents.length - connected.length} not connected`}</p></div>
      <div><dt>Access provisioned</dt><dd>{stale ? 'Unavailable' : <>{provisioned}<small> / {agents.length}</small></>}</dd><p>{stale ? 'Credential status cannot be confirmed' : `${agents.length - provisioned} without valid credentials`}</p></div>
      <div><dt>Reported work</dt><dd>{stale ? 'Unavailable' : working}<small>{!stale && ' working'}</small></dd><p>{stale ? 'Activity cannot be confirmed' : `${waiting} waiting / attention · ${unknown} unreported`}</p></div>
    </dl>
