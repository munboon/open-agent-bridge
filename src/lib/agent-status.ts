...
export function agentStatus(input: {
  ...
  return { contact, connection, work, provisioned: input.valid_credential, reason, task_title: input.task_title, last_seen_at: seen?.toISOString() ?? null, sampled_at: now.toISOString() };
}
