...
export type PortalLocation={area?:'administration'|'account';project:string|null;section:typeof sections[number];thread?:string;invalid?:boolean};
...
export function portalPath(location:PortalLocation){return location.area?'/'+location.area:location.project?`/projects/${location.project}/${location.section}${location.thread?'/'+location.thread:''}`:'/projects';}
